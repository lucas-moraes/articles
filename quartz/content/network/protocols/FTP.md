...
**Modo Ativo**: O cliente abre uma porta e o servidor se conecta de volta a ela para transferir os dados. _Problema_: Firewalls no computador do cliente costumam bloquear conexões vindas de fora, quebrando o FTP.
...
\*_FTPS (FTP over SSL/TLS)_: É o mesmo protocolo FTP, mas que adiciona uma camada de criptografia idêntica à do HTTPS (SSL/TLS). Ele protege os dados, mas mantém a estrutura de duas conexões (canais de controle e dados).
